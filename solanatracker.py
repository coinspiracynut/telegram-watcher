...
class SolanaTracker:
    ...
    async def _transaction_sender_and_confirmation_waiter(
        ...
        # Resend options always skip preflight (state may have changed since first send)
        resend_tx_opts = TxOpts(
            skip_preflight=True,
            preflight_commitment=tx_preflight_commitment_for_tx_opts,
            max_retries=send_opts_dict.get("max_retries") # type: ignore
        )
        ...
        try: 
            ...
            resend_interval_s = options.get("resend_interval_ms", DEFAULT_TRANSACTION_OPTIONS["resend_interval_ms"]) / 1000.0 # type: ignore
            
            self.logger.info(
                f"Waiting for confirmation for {signature} "
                f"(desired: {desired_commitment_obj}, up to {retries} checks @ {check_interval_s}s, "
                f"resend every {resend_interval_s}s)..."
            )

            time_since_last_resend = 0.0

            for attempt in range(retries + 1): # type: ignore
                await asyncio.sleep(check_interval_s)
                time_since_last_resend += check_interval_s

                # --- Resend periodically to increase landing probability ---
                if time_since_last_resend >= resend_interval_s:
                    try:
                        await connection.send_raw_transaction(
                            signed_transaction_bytes, opts=resend_tx_opts
                        )
                        time_since_last_resend = 0.0
                        self.logger.info(f"🔄 Resent tx (attempt {attempt + 1}/{retries + 1})")
                    except Exception as resend_e:
                        # Resend failures are non-fatal (tx might already be processed,
                        # or the RPC might rate-limit us — either way, keep checking status)
                        self.logger.debug(f"Resend non-fatal: {resend_e}")
                ...
                try:
                    ...
                    if status_resp.value and status_resp.value[0] is not None:
                        ...
                        if sig_status.err:
                            ...
                            self.logger.error(f"❌ Transaction failed on-chain: {err_obj}")
                            return err_obj
                        ...
                        if sig_status.confirmation_status:
                            ...
                            if self._is_commitment_level_sufficient(current_enum, desired_conf_status_enum):
                                elapsed = (attempt + 1) * check_interval_s
                                self.logger.info(f"✅ Tx {signature} confirmed: {current_enum} (after ~{elapsed:.0f}s)")
                                return str(signature)
                            else:
                                self.logger.debug(f"Tx status: {current_enum} (need {desired_conf_status_enum})")
                    else:
                        self.logger.debug(f"Status not yet available (attempt {attempt + 1}/{retries + 1})")

                except Exception as status_e: 
                    self.logger.warning(f"Status check error (attempt {attempt + 1}): {status_e}")
                    # Continue — transient RPC errors shouldn't abort

                # --- Block height expiry check (every 5th attempt to save RPC calls) ---
                if (attempt + 1) % 5 == 0:
                    try:
                        bh_resp: GetBlockHeightResp = await connection.get_block_height(commitment=Processed)
                        current_bh = bh_resp.value
                        self.logger.debug(f"Block height: {current_bh}, tx valid until ~{effective_lvbh}")
                        if current_bh > effective_lvbh:
                            err_obj = TransactionExpiredError(
                                str(signature),
                                f"Blockhash expired (block {current_bh} > {effective_lvbh})"
                            )
                            self.logger.error(f"⏰ {err_obj}")
                            return err_obj
                    except Exception as bh_e:
                        self.logger.debug(f"Block height check non-fatal: {bh_e}")
            
            total_time = (retries + 1) * check_interval_s
            timeout_err_obj = TransactionConfirmationTimeoutError(
                str(signature),
                f"Not confirmed after {total_time:.0f}s ({retries + 1} checks)"
            )
            self.logger.error(f"⏰ {timeout_err_obj}")
            return timeout_err_obj

        except Exception as e: 
            self.logger.error(f"Error in tx sender/waiter: {e}", exc_info=True)
